...
internal class Manhastro(context: MangaLoaderContext) :
	...
	override suspend fun getListPage(page: Int, order: SortOrder, filter: MangaListFilter): List<Manga> {
		...
		if (!filter.query.isNullOrEmpty()) {
			...
			filtered = filtered.filter { item ->
				(item.getStringOrNull("titulo")?.lowercase(sourceLocale)?.contains(query) == true) ||
					(item.getStringOrNull("titulo_brasil")?.lowercase(sourceLocale)?.contains(query) == true)
			}
		}
		filter.tags.firstOrNull()?.let { tag ->
			filtered = filtered.filter { item ->
				item.genreList().any { it.equals(tag.key, ignoreCase = true) }
			}
		}
	...
	override suspend fun getDetails(manga: Manga): Manga {
		val mangaId = manga.url.trimStart('/').substringAfterLast('/')
		val cached = allMangasCache.get().firstOrNull {
			it.optInt("manga_id", -1).toString() == mangaId
		}
		...
		val chapters = chaptersArr.mapJSONNotNull { chapter ->
			val chapterId = chapter.optInt("capitulo_id", -1).takeIf { it > 0 } ?: return@mapJSONNotNull null
			val name = chapter.getStringOrNull("capitulo_nome") ?: "Capítulo"
			...
			MangaChapter(
				...
				url = url,
				scanlator = null,
				uploadDate = chapter.getStringOrNull("capitulo_data")
		...
		val altTitle = cached?.getStringOrNull("titulo_brasil")
	...
	override suspend fun getPages(chapter: MangaChapter): List<MangaPage> {
		...
		val json = webClient.httpGet("$apiBase/paginas/$chapterId").parseJson()
	...
	private fun mangaFromJson(obj: JSONObject): Manga {
		...
		val altTitle = obj.getStringOrNull("titulo_brasil")
			?.takeUnless { it.equals(title, ignoreCase = true) }
		val cover = obj.getStringOrNull("imagem")?.let(::normalizeCoverUrl)
